 ...
 * 「前日上り」（ゴール前200mのタイム・秒）を昇順に並べ、レースごとの上位N人を抽出する。
 */
    ...
    for (const meet of data) {
        ...
        if (!meetHasData(meet)) {
            parts.push(`<div class="meet">
                ...
                <div class="meet-body">
                    <div class="no-data">
                        <strong>前日上りのデータなし</strong>
                        ${Number(meet.race_day) === 1
                            ? '初日（1日目）のため前日成績がありません。'
                            : '前日上りが記録されている選手がいません。'}
                    </div>
                </div>
            </div>`);
    ...
    for (const meet of data) {
        ...
        for (const race of sortedRaces(meet)) {
            ...
            const body = ranked.length > 0
                ...
                : `<div class="no-data">このレースには前日上りの記録がありません。</div>`;
        ...
        parts.push(`<div class="meet">
            ${meetHeader(meet)}
            <div class="meet-body">
                ${hasData ? '' : `<div class="no-data"><strong>前日上りのデータなし</strong>${
                    Number(meet.race_day) === 1
                        ? '初日（1日目）のため前日成績がありません。出走表のみ表示します。'
                        : '前日上りが記録されている選手がいません。'
                }</div>`}
                ${blocks.join('')}
            </div>
